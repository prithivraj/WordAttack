 ...
 */
@SuppressLint("CommitPrefEdits") //No i18n
public final class SettingUtils {

    private final static boolean DEBUG = true;
    private final static String TAG = "SettingUtils"; //No i18n
    ...
    private static Method findApplyMethod() {
        try {
            ...
            return cls.getMethod("apply"); //No i18n
        } catch (NoSuchMethodException unused) {
            if (DEBUG) {
                LogUtils.d(TAG, "Failed to retrieve Editor.apply(); probably doesn't exist on this phone's OS.  Using Editor.commit() instead."); //No i18n
            }
            return null;
        }
    }
    ...
    public static void commitOrApply(Editor editor) {
        if (APPLY_METHOD != null) {
            try {
                ...
                if (DEBUG) {
                    LogUtils.d(TAG, "Failed while using Editor.apply().  Using Editor.commit() instead."); //No i18n
                }
